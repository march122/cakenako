...
// Example of receiving filter parameter from the URL
$filter = isset($_GET['filter']) ? $_GET['filter'] : 'all'; // Default to 'all' if no filter is selected

// Fetch cakes based on the filter
if ($filter === 'all') {
    // Fetch all cakes from the database (or your cakes array)
    $cakes = $cakesByCategory;
} else {
    // Fetch cakes specific to the selected category
    $cakes = isset($cakesByCategory[$filter]) ? $cakesByCategory[$filter] : [];
}
...
<!-- jQuery, Popper.js, and Bootstrap JS (Order matters) -->
...
<div class="dropdown">
    ...
    <div class="dropdown-menu" aria-labelledby="categoryDropdown">
        <a class="dropdown-item <?= $filter === 'all' ? 'active' : '' ?>" href="?filter=all" role="tab" data-toggle="tab">All Cakes</a>
        <?php foreach ($latestImages as $category => $image) : ?>
            <a class="dropdown-item <?= $filter === $category ? 'active' : '' ?>" href="?filter=<?= strtolower(str_replace(' ', '', $category)); ?>" role="tab" data-toggle="tab"><?= htmlspecialchars($category); ?></a>
        <?php endforeach; ?>
    </div>
</div>
        ...
        <div class="tab-content" id="myTabContent">
            ...
            <div class="tab-pane fade <?= $filter === 'all' ? 'show active' : '' ?>" id="allcakes" role="tabpanel">
                <div class="tab-single">
                    <div class="row">
                        <?php foreach ($cakes as $category => $cakesInCategory) : ?>
                            <?php foreach ($cakesInCategory as $cake) : ?>
            ...
            <!-- Category-specific Cakes -->
            <?php foreach ($cakesByCategory as $category => $cakesInCategory) : ?>
                <div class="tab-pane fade <?= $filter === strtolower(str_replace(' ', '', $category)) ? 'show active' : '' ?>" id="<?= strtolower(str_replace(' ', '', $category)); ?>" role="tabpanel">
                    <div class="tab-single">
                        <div class="row">
                            <?php foreach ($cakesInCategory as $cake) : ?>
                                <div class="col-xl-3 col-lg-4 col-md-4 col-12">
...
$(document).ready(function () {
    // Event listener for clicking a category in the dropdown
    $('.dropdown-item').click(function (e) {
        var categoryId = $(this).attr('href'); // Get the category id from href
        $('.tab-pane').removeClass('show active'); // Remove active class from all tabs
        $(categoryId).addClass('show active'); // Add active class to the clicked tab
    });
});
</script>
